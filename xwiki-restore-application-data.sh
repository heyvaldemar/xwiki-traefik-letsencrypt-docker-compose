...
BACKUP_PATH="/srv/xwiki-application-data/backups/"
RESTORE_PATH="/usr/local/xwiki/"
BACKUP_PREFIX="xwiki-application-data"

echo "--> All available application data backups:"

for entry in $(docker container exec -it "$XWIKI_BACKUPS_CONTAINER" sh -c "ls $BACKUP_PATH")
do
  echo "$entry"
done
...
docker exec -it "$XWIKI_BACKUPS_CONTAINER" sh -c "rm -rf ${RESTORE_PATH}* && tar -zxpf ${BACKUP_PATH}${SELECTED_APPLICATION_BACKUP} -C /"
