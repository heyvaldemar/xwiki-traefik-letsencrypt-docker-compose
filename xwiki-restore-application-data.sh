#!/usr/bin/env bash
# xwiki-restore-application-data.sh [backup-file-name]
#
# Replaces XWiki's application data with one of the archives the backups
# service wrote.
#
#   ./xwiki-restore-application-data.sh               list and ask
#   ./xwiki-restore-application-data.sh <file-name>   restore that one
#
...
if ! docker exec "$BKP" sh -c "set -eu
  ...
  echo "error: the restore failed part-way; $DATA may be incomplete. Restore another archive before using XWiki." >&2
  exit 1
fi
