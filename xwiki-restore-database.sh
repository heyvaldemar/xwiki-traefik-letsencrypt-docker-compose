#!/usr/bin/env bash
# xwiki-restore-database.sh [backup-file-name]
#
# Replaces the XWiki database with one of the dumps the backups service wrote.
#
#   ./xwiki-restore-database.sh               list and ask
#   ./xwiki-restore-database.sh <file-name>   restore that one
#
...
#
# Set COMPOSE_PROJECT_NAME if the stack was started with a -p other than xwiki.
set -Eeuo pipefail

PROJECT="${COMPOSE_PROJECT_NAME:-xwiki}"
APP_SERVICE="xwiki"
...
DB_NAME="$(env_of XWIKI_DB_NAME)"; DB_USER="$(env_of XWIKI_DB_USER)"
...
if ! docker exec "$BKP" sh -c "(set -o pipefail) 2>/dev/null && set -o pipefail; set -eu
  ...
  echo "error: the restore failed part-way. The database may now be empty: restore another backup before using XWiki." >&2
  exit 1
fi
